...
export default function BrowseByJudgeDetails({ route, navigation }) {
    ...
    console.log("data frm previous", route.params);

    const judgesNameList = judgesname.join("&judges=");
    console.log("judgesNameList", judgesNameList);
    ...
    const JudgeDigestView = async () => {
        ...
        try {
            ...
            const response = await getJudgeDetails(judgesname, or, and, 10, `${offset}`);
    ...
    const renderShortNote = ({ item, index, citationID }) => {
        ...
        return (
            <View style={globalStyle.noteContainer}>
                ...
                {highlightText(snoteText, judgesNameList)}
                {isExpanded &&
                    // <Text style={globalStyle.noteLText}>{item.lnote}</Text>
                    highlightText(lnoteText, judgesNameList)
                }
                {/* {item.snote && (
    ...
    const CourtFilterDigestView = async (courts, judges, benchStrength, Case_Results, Decision_Years, Nominal, nominalRes, topics, acts, section, searchInSearch, searchFilter) => {
        if (loading) return;
        setLoading(true);
        try {
            ...
            if (judges) {
                ...
                console.log("judges name values", judgesname.concat(judges));
                judgesnamenew = judgesname.concat(judges);

            } else {
                judgesnamenew = judges;
            }
            const response = await getJudgeDetails(judgesnamenew, or, and, 10, `${page}`, courts, benchStrength, Case_Results, Decision_Years, Nominal, nominalRes, topics, acts, section, searchInSearch, searchFilter);
            console.log("getJudgeDetails", response);
        ...
        <SafeAreaView edges={['left', 'right', 'bottom']} style={globalStyle.safearea}>
            <Background>
                ...
                <CustomModal
                    ...
                    data={{ ...(filterData || {}), judgesNameList }}
                />
