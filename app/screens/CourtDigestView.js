...
export default function CourtDigestView({ route, navigation }) {
    ...
    const DigestView = async () => {
        if (loading || !hasMore) return;
        setLoading(true);
        try {

            const response = await getFTSDigestView('', 10, `${offset}`, courtlist);
    ...
    const CourtFilterDigestView = async (courts, judges, benchStrength, Case_Results, Decision_Years, Nominal, nominalRes, topics, acts, section, searchInSearch) => {
        ...
        try {
            ...
            const response = await getFTSDigestView(courtlist, 10, `${page}`, courts, judges, benchStrength, Case_Results, Decision_Years, Nominal, nominalRes, topics, acts, section, searchFilter, searchInSearch);
            // console.log("getFTSDigestView", response);
